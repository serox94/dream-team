...
async function bootstrap(env){
  ...
  for(const t of trips){
    const s=await env.DB.prepare(`SELECT COUNT(*) fishCount,COALESCE(SUM(weight_kg),0) totalWeightKg,COALESCE(MAX(weight_kg),0) biggestFishKg FROM catches WHERE trip_id=?`).bind(t.id).first();
    const b=await env.DB.prepare(`SELECT a.name anglerName FROM catches c JOIN anglers a ON a.id=c.angler_id WHERE c.trip_id=? ORDER BY c.weight_kg DESC,c.caught_at ASC LIMIT 1`).bind(t.id).first();
    const p=await env.DB.prepare(`SELECT spot,COUNT(*) cnt FROM catches WHERE trip_id=? AND spot IS NOT NULL AND TRIM(spot)<>'' GROUP BY spot ORDER BY cnt DESC LIMIT 1`).bind(t.id).first();
    out.push({...t,isActive:Boolean(t.isActive),facts:JSON.parse(t.factsJson||'{}'),stats:{fishCount:Number(s?.fishCount||0),totalWeightKg:Number(s?.totalWeightKg||0),biggestFishKg:Number(s?.biggestFishKg||0),biggestFishAngler:b?.anglerName||null,bestSpot:p?.spot||null}})
  }
...
async function listCatches(r,env){const tripId=new URL(r.url).searchParams.get('tripId');if(!tripId)return bad('tripId is required');const q=await env.DB.prepare(`SELECT c.id,c.trip_id tripId,c.angler_id anglerId,a.name anglerName,c.caught_at caughtAt,c.weight_kg weightKg,c.species,c.spot,c.bait,c.rig,c.depth_m depthM,c.notes,c.photo_url photoUrl FROM catches c JOIN anglers a ON a.id=c.angler_id WHERE c.trip_id=? ORDER BY c.caught_at DESC,c.id DESC`).bind(tripId).all();return json({ok:true,catches:q.results})}
async function createCatch(r,env){const x=await body(r);if(!x)return bad('Invalid JSON');const w=Number(x.weightKg);if(!x.tripId||!x.anglerId||!Number.isFinite(w)||w<=0)return bad('tripId, anglerId and positive weightKg are required');const q=await env.DB.prepare(`INSERT INTO catches(trip_id,angler_id,caught_at,weight_kg,species,spot,bait,rig,depth_m,notes,photo_url) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(x.tripId,x.anglerId,x.caughtAt||new Date().toISOString(),w,x.species||'Karp',x.spot||null,x.bait||null,x.rig||null,x.depthM??null,x.notes||null,x.photoUrl||null).run();return json({ok:true,id:q.meta.last_row_id},201)}
...
const IMPORT_TOKEN='dt-import-9d7f4c2a';
...
async function importOld(r,env){
  if(new URL(r.url).searchParams.get('token')!==IMPORT_TOKEN)return json({ok:false,error:'Forbidden'},403);
  const done=await env.DB.prepare("SELECT value FROM app_settings WHERE key='supabase_import_v1'").first();if(done)return json({ok:true,alreadyImported:true,summary:JSON.parse(done.value)});
  const [checks,spots,catches]=await Promise.all([oldTable('checklist_items'),oldTable('spots'),oldTable('catches')]);
  let ci=0,si=0,fi=0;
  ...
  for(const x of catches){const ang=String(x.person||'').toLowerCase().includes('mac')?'maciek':'patryk';const w=Number(x.weight);if(!(w>0))continue;await env.DB.prepare(`INSERT INTO catches(trip_id,angler_id,caught_at,weight_kg,species,spot,bait,notes) VALUES('la-plaine-2026',?,?,?,?,?,?,?)`).bind(ang,x.caught_at||new Date().toISOString(),w,x.species||'Karp',x.spot||null,x.bait||null,[x.note,'Import z Ryby 2026 / Supabase'].filter(Boolean).join(' · ')).run();fi++}
  const summary={checklist:ci,spots:si,catches:fi,importedAt:new Date().toISOString()};await env.DB.prepare("INSERT OR REPLACE INTO app_settings(key,value,updated_at) VALUES('supabase_import_v1',?,CURRENT_TIMESTAMP)").bind(JSON.stringify(summary)).run();return json({ok:true,summary})
}

export default{async fetch(r,env){const u=new URL(r.url);try{
  if(u.pathname==='/api/health'&&r.method==='GET'){const v=await env.DB.prepare("SELECT value FROM app_settings WHERE key='schema_version'").first();return json({ok:true,app:'dream-team',database:'connected',schemaVersion:v?.value||null})}
  if(u.pathname==='/api/bootstrap'&&r.method==='GET')return json(await bootstrap(env));
  if(u.pathname==='/api/admin/import-old'&&r.method==='GET')return importOld(r,env);
  if(u.pathname==='/api/catches'&&r.method==='GET')return listCatches(r,env);if(u.pathname==='/api/catches'&&r.method==='POST')return createCatch(r,env);
  let m=u.pathname.match(/^\/api\/catches\/(\d+)$/);if(m&&r.method==='DELETE'){await env.DB.prepare('DELETE FROM catches WHERE id=?').bind(+m[1]).run();return json({ok:true})}
  if(u.pathname==='/api/spots'&&r.method==='GET')return listSpots(r,env);if(u.pathname==='/api/spots'&&r.method==='POST')return createSpot(r,env);m=u.pathname.match(/^\/api\/spots\/(\d+)$/);if(m&&r.method==='DELETE'){await env.DB.prepare('DELETE FROM spots WHERE id=?').bind(+m[1]).run();return json({ok:true})}
  if(u.pathname==='/api/checklist'&&r.method==='GET')return listChecklist(r,env);if(u.pathname==='/api/checklist'&&r.method==='POST')return createChecklist(r,env);m=u.pathname.match(/^\/api\/checklist\/(\d+)$/);if(m&&r.method==='PATCH')return patchChecklist(r,env,+m[1]);if(m&&r.method==='DELETE'){await env.DB.prepare('DELETE FROM checklist_items WHERE id=?').bind(+m[1]).run();return json({ok:true})}
  if(u.pathname==='/api/documents'&&r.method==='GET')return documents(r,env);
  m=u.pathname.match(/^\/api\/trips\/([^/]+)$/);if(m&&r.method==='PUT')return updateTrip(r,env,decodeURIComponent(m[1]));m=u.pathname.match(/^\/api\/trips\/([^/]+)\/activate$/);if(m&&r.method==='POST')return activate(env,decodeURIComponent(m[1]));
  if(u.pathname.startsWith('/api/'))return nf();return env.ASSETS.fetch(r)
}catch(e){console.error(e);return json({ok:false,error:'Internal server error',detail:String(e?.message||e)},500)}}};